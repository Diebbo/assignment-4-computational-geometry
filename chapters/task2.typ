...
We want to show an example of a polygon with holes where, given a point $q$ inside the polygon, there exists a point $q'$ such that the geodesic distance is maximum and $q'$ is not on the boundary of the polygon.
...
Let's denote $T_u$ the upper equilateral triangle where $q'$ lies. From definition all the vertices are equally distant from each other and from the center.
...
In addition, the x-coordinate of $q$ is in the midpoint of the horizontal hole that defines $T_b$, so the two shortest paths to the edges of the hole are equal.
...
Finally, the point $q'$ that maximizes the geodesic distance from $q$ is the center of the triangle $T_u$, since it is equally distant from all the vertices of the triangle, and every point on the boundaries would be closer to at least one of the vertices, by the Pythagorean theorem.

To be more precise, for visualization purposes, the image concedes some approximation, since we need to show that the holes are actually disjoint rectangles, and the triangles are not perfectly equilateral, but at least isosceles. However, the reasoning still holds.
